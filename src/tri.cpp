...
)
{
    ...
    for (int i=bbox_min_y ; i<bbox_max_y && i<WINDOW_HEIGHT ; i++)
    {
        for (int j=bbox_min_x ; j<bbox_max_x && j<WINDOW_WIDTH ; j++)
        {
            ...
            {
                frame_image[(WINDOW_WIDTH*i+j)*4] = 200;
                frame_image[(WINDOW_WIDTH*i+j)*4 + 1] = 200;
                frame_image[(WINDOW_WIDTH*i+j)*4 + 2] = 200;
                frame_image[(WINDOW_WIDTH*i+j)*4 + 3] = 255;
...
)
{
    ...
    for (int i=bbox_min_y ; i<bbox_max_y && i<WINDOW_HEIGHT ; i++)
    {
        for (int j=bbox_min_x ; j<bbox_max_x && j<WINDOW_WIDTH ; j++)
        {
            ...
            {
                frame_image[(WINDOW_WIDTH*i+j)*4] = u*255.0f;
                frame_image[(WINDOW_WIDTH*i+j)*4 + 1] = v*255.0f;
                frame_image[(WINDOW_WIDTH*i+j)*4 + 2] = w*255.0f;
                frame_image[(WINDOW_WIDTH*i+j)*4 + 3] = 255;
        ...
        for (int j=bbox_min_x ; j<bbox_max_x && j<WINDOW_WIDTH ; j++)
        {
            ...
            )
            {
                ...
                if (texel_x < texture_width && texel_y < texture_height)
                {
                    ...
                    frame_image[(WINDOW_WIDTH*i+j)*4] = red;
                    frame_image[(WINDOW_WIDTH*i+j)*4 + 1] = green;
                    frame_image[(WINDOW_WIDTH*i+j)*4 + 2] = blue;
                    frame_image[(WINDOW_WIDTH*i+j)*4 + 3] = 255;
